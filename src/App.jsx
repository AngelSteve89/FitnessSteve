...
export default function App() {
  // NEW: date selector for viewing/logging any day
  ...
  const todaysPushups = useMemo(() => {
    const dayEntries = state.pushups.filter(p => p.date === selectedDate)
    return sum(dayEntries.map(p => p.count))
  }, [state.pushups, selectedDate])

  // ---- Derived: meals ----
  const todaysMeals = useMemo(() => mealsForDate(state.meals, selectedDate), [state.meals, selectedDate])
  ...
  function addWeight(e) {
    ...
    if (!weight) return
    setState(s => ({ ...s, weights: [...s.weights, { date: selectedDate, weight }] }))
    e.target.reset()
  }

  function addPushupsCount(n) {
    setState(s => ({ ...s, pushups: [...s.pushups, { date: selectedDate, count: n }] }))
  }
  ...
  function undoLastPushups() {
    setState(s => {
      const i = s.pushups.slice().reverse().findIndex(p => p.date === selectedDate)
      if (i === -1) return s
  ...
  function addRide(e) {
    ...
    const ride = {
      date: form.get("date") || selectedDate,
  ...
  function quickAddMeal(preset) {
    const meal = { date: selectedDate, ...preset }
    setState(s => ({ ...s, meals: [meal, ...s.meals] }))
  }
  ...
  function loadDemo() {
    ...
    const demo = {
      ...
      rides: [
        { date: today, title: "20-min Pop Ride", minutes: 20, output: 235 },
        { date: "2025-10-29", title: "30-min Climb", minutes: 30, output: 310 }
      ],
      meals: [
        { date: today, name: "Factor: Grilled Chicken", calories: 520, protein: 45, carbs: 34, fats: 21 },
        { date: today, name: "Protein Shake", calories: 180, protein: 30, carbs: 6, fats: 3 },
      ]
    }
    setState(demo)
      ...
      <div style={container}>
          ...
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:8, flexWrap:"wrap"}}>
            ...
            <div style={{opacity:.85}}>
              <input
                ...
                style={{background:"#0b1220", color:"#e6ecff", border:"1px solid #243043", borderRadius:8, padding:"8px 10px"}}
              />
            </div>
          </div>
            ...
            <div style={{background:"#0f172a", border:"1px solid #1f2937", borderRadius:12, padding:12}}>
              ...
              <form onSubmit={addRide} style={{display:"grid", gap:8}}>
                ...
                <div style={row}>
                  ...
                  <input name="date" type="date" defaultValue={selectedDate} style={{...input, flex:"1 1 140px"}}/>
                </div>
        ...
        <div style={{...card, marginBottom:16}}>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
            ...
            <div style={{opacity:.75}}>{formatDate(selectedDate)}</div>
          </div>
